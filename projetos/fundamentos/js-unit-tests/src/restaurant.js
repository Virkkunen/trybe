...
/* Esse deve ser o exercício mais mal feito e escrito que já vi */
...
const createMenu = (menu) => {
  const result = {
    ...
    pay: () => {
      let price = 0;
      for (let i = 0; i < result.consumption.length; i += 1) {
        if (Object.prototype.hasOwnProperty.call(result.fetchMenu().food, result.consumption[i])) {
          price += result.fetchMenu().food[result.consumption[i]];
        }
        if (Object.prototype.hasOwnProperty.call(result.fetchMenu().drink, result.consumption[i])) {
          price += result.fetchMenu().drink[result.consumption[i]];
        }
      }
      return price * 1.1;
    },
...
const orderFromMenu = (item) => createMenu().order(item);
